...
public class Level2 implements KeyListener,Runnable{
    ...
    public void createLevel2(){
        ...
        for(int j=100;j<= 210; j+=40){
            for(int k= 20;k<=850;k+=80){
                if(brickCount > 10 && brickCount <= 21){
    ...
    public void isCollidePaddle(JLabel ball,JLabel paddle){
        ...
        if(paddle.getBounds().intersects(ball.getBounds())){
            System.out.println("paddle x and y; " + paddle.getX() +" " + paddle.getY());
            yBallSpd = -yBallSpd;
        }
	...
	public void keyPressed(KeyEvent event) {
		...
		if (whichKey.compareTo("Left") == 0) {
            ...
            JOptionPane.showMessageDialog(null,"You completed the level 2.Next is Level 3!!");
			isgameWon = true;
            frame.dispose();
            Level3 gmm = new Level3();
            gmm.createLevel3();                            ////THIS IS FOR DEVELOPER ONLY
            gmm.score = score;
            gmm.lives = lives;
