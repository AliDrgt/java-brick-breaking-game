...
public class Level1 implements KeyListener,Runnable{
    ...
    public void createLevel1(){
        ...
        for(int k= 20;k<=850;k+=80){
            for(int j=100;j<= 210; j+=40){
                Bricks n1 = new Bricks(new ImageIcon(new ImageIcon((main.class.getResource("redBrick.png"))).getImage().getScaledInstance(45,20, Image.SCALE_DEFAULT)));
                n1.setBounds(k, j, 45, 20);
                brickArray.add(n1);
                frame.add(n1);
            }
        }
	...
	public void keyPressed(KeyEvent event) {
		...
		if (whichKey.compareTo("Left") == 0) {
            ...
            JOptionPane.showMessageDialog(null,"You completed the level 1.Next is Level 2!!");
			isgameWon = true;
            frame.dispose();
            Level2 gmm = new Level2();
            gmm.createLevel2();                              ////THIS IS FOR DEVELOPER ONLY
            gmm.score = score;
            gmm.lives = lives;
